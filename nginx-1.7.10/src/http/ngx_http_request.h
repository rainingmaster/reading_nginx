...
//ngx_http_request_t中的method，使用数字方便对比
...
typedef struct {
    ngx_str_t                         name; //每项的名称字符串，如content_type
...
/* 请求头结构体。 */
typedef struct {
    ngx_list_t                        headers; //使用list存储请求头
...
struct ngx_http_request_s {
    ...
    */
    ...
    //当前请求初始化时的时间
    time_t                            start_sec;//秒部分
    ngx_msec_t                        start_msec;//微秒部分
    ...
    //状态机解析http时使用state来表示当前的解析状态，多用在反复调用一个函数时，存储其处理状态，如ngx_http_parse_header_line中
    ngx_uint_t                        state;

    ngx_uint_t                        header_hash;
    ngx_uint_t                        lowcase_index;

    //都是记录ngx_http_parse_header_line多次解析时的一些中间变量
    //每次循环解析中，记录小写的请求头字符串。只有32位(头名称不得超过32位)
